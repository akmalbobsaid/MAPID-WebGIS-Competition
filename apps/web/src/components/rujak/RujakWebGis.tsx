...
import { discover, getIsochrone, getMerchant, getStop, getStops, RujakApiError } from "@/lib/client/rujak-api";
import type { DiscoveryResponse, Isochrone, MerchantDetail as MerchantDetailType, MerchantResult, StopDetail, StopSummary } from "@/lib/client/rujak-types";
...
const initialState: State = {
  ...
  unfiltered: null, selectedMerchantId: null, merchant: idle<MerchantDetailType>(), viewportRevision: 0, retryVersion: 0,
};
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "SELECT_STOP": return { ...state, selectedStopId: action.stopId, categoryL2: null, selectedMerchantId: null, merchant: idle(), stop: idle(), isochrone: idle(), discovery: idle(), unfiltered: null, viewportRevision: state.viewportRevision + 1 };
...
export default function RujakWebGis() {
  ...
  const busy = state.discovery.status === "loading" || state.isochrone.status === "loading";
